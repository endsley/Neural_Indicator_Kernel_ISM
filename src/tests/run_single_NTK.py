...
from src.ntk.run_NTK import *
from src.networks.GP_debug import *
import sklearn.metrics



def run_single_NTK(data_name, KN, loss_function):
	db = {}
	...
	for i in [0]:						# loop for indentation readability purpose only
		[db['train_acc'], db['test_acc']] = run_NTK(db['train_data'].X, db['train_data'].Y, db['test_data'].X ,db['test_data'].Y)

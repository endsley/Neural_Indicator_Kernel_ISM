...
def run_single_DeepGP(data_name, KN, loss_function):
	db = {}
	...
	for i in [0]:						# loop for indentation readability purpose only
		[db['train_acc'], db['test_acc']] = Deep_Kernel_Gaussian_Process(db['train_data'].X, db['train_data'].Y, db['test_data'].X ,db['test_data'].Y)
